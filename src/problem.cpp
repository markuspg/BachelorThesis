...
bct::Problem::Problem(const std::string &argFilename) {
  ...
  // Read the lines, store them as str and convert it into the variables (waste
  // is used to omit commentaries)
...
bool bct::Problem::CheckValidity() {
  for (unsigned short j = 0; j < processesQuantity; ++j) {
    ...
    if (processes[j]->GetAssignedMachinesId() > machinesQuantity) {
      // std::cout << "\t  Process " << processes[j]->GetId()
      //           << " is not assigned to a valid machine => no feasible
      //           solution.\n";
      return false;
    }
  }
...
unsigned int bct::Problem::QueryLowestCompletionTime() {
  auto lowestCompletionTimeIt = std::min_element(
      machines.cbegin(), machines.cend(), [](const auto argA, const auto argB) {
...
/*!
 * \brief Query the id of the Machine with the shortest or longest completion
 * time \param[in] argPlacement The first (=0), second (=1) or x-th Machine
 * fullfilling the requirement \param[in] argInvert True, if the Machine with
 * the longest completion time shall be returned \return The id of the Machine
 * with the shortest (longest) completion time
 */
unsigned short
bct::Problem::QueryLowestWorkloadMachinesId(const unsigned short argPlacement,
                                            const bool argInvert) {
  ...
  // Pop the Machine with the lowest or highest workload until 'pop_machines' is
  // empty
  while (popMachines.empty() == false) {
    ...
    // Search over the complete array starting by index 1, since 0 will never
    // become true
  ...
  /* for (auto it = push_machine_ids.cbegin (); it != push_machine_ids.cend ();
  ++it) { std::cout << "Machine " << *it << " with workload " << machines[*it -
  1]->get_completion_time () << "\n";
  } */
...
void bct::Problem::QueryState() {
  ...
  // Query the Machine's Processes after having checked, if the current solution
  // is feasible
  if (CheckValidity() == true) {
    for (unsigned int i = 0; i < machinesQuantity; ++i) {
      ...
      // Iterate over all assigned Processes and output the ids and processing
      // times
...
/*!
 * \brief Save the current solution in the TemporaryStorage especially designed
 * therefore
 */
