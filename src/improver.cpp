 ...
 */
bct::Improver::Improver(const Problem &argProblem) : Problem{argProblem} {
...
unsigned int
bct::Improver::Apply_PAIRWISE_algorithm(const unsigned short argIterations,
                                        const bool argGreedy) {
  ...
  while (stop == false) {
    for (unsigned short k = 0; k < machinesQuantity - 1; ++k) {
      ...
      // Stop, if the upper bound correlates to the current solution value (Step
      // 5: If LB = M)
      if (upperBound == this->QueryLowestCompletionTime()) {
        // std::cout << "Current schedule correlates to upper bound =>
        // Terminating.\n";
        return iter;
      }

      // Stop if all machines have the same load (Step 6: Terminate if F_PA =
      // F_PB)
      ...
      auto maximumDifference =
          pa->GetCompletionTime() - pb->GetCompletionTime();
      ...
      for (auto ita = aProcesses.cbegin(); ita != aProcesses.cend(); ++ita) {
        for (auto itb = bProcesses.cbegin(); itb != bProcesses.cend(); ++itb) {
          if (!argGreedy) {
            // Allowing <= yielding in no advantageous swaps for greater
            ...
            // Not using <= in both cases, because it does not lead to an
            // advantageous swap
            if ((*ita)->GetProcessingTime() > (*itb)->GetProcessingTime()) {
              if (((*ita)->GetProcessingTime() - (*itb)->GetProcessingTime()) <
                  maximumDifference) {
      ...
      for (auto ita = setA.cbegin(), itb = setB.cbegin(); ita != setA.cend();
           ++ita, ++itb) {
    ...
    if (setA.size() > 0) {
      ...
      if (argGreedy == false) {
        ...
        for (auto ita = setA.cbegin(), itb = setB.cbegin(); ita != setA.cend();
          ...
          if (((*ita)->GetProcessingTime() - (*itb)->GetProcessingTime()) >
              bestSwap) {
            bestSwap =
                (*ita)->GetProcessingTime() - (*itb)->GetProcessingTime();
            swapIndex = counter;
          }
          counter++;
        }
      ...
      // Do the swap and compute new finish times (Step 9: Exchange two jobs a
      // and b)
    ...
    // new improvable solutions
      ...
      // std::cout << "No advantageous swaps can be found anymore,
      // terminating.\n";
      stop = true;
    }
    ++iter;
    if (argGreedy == false) {
      if (iter >= argIterations) {
        stop = true;
      }
    }
  }
 ...
 * \param argIterations The count of iterations the algorithm shall be applied
 * (only affects non-deterministic algorithms)
 */
unsigned int
bct::Improver::ImproveStartSolution(const EImprovementAlgos argAlgo,
                                    const unsigned short argIterations) {
  switch (argAlgo) {
  case EImprovementAlgos::gPAIRWISE:
    return Apply_PAIRWISE_algorithm(argIterations, true);
    break;
  case EImprovementAlgos::PAIRWISE:
    return Apply_PAIRWISE_algorithm(argIterations);
    break;
  default:
