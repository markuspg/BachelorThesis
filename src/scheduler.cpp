 ...
 */
bct::Scheduler::Scheduler (const Problem &argProblem):
    Problem{argProblem}
{
...
/*!
 * \brief Applies the intelligent LPT (assign longest unassigned job to the machine with lowest workload) algorithm to create a starting solution for the given instance stored in the Scheduler
 */
...
/*!
 * \brief Applies the randomized LPT (assign longest or second-longest unassigned job to the next free machine) algorithm to create a starting solution for the given instance stored in the Scheduler
 * \param[in] argIterations The quantity of to-be-conducted randomized solution attempts
 */
void bct::Scheduler::Apply_rLPT_Algorithm(const unsigned short argIterations) {
...
/*!
 * \brief Applies the bin covering SIMPLE (SI) algorithm from Csirik, Frenk, Labbe and Zhang (1999) to create a starting solution for the given instance stored in the Scheduler
 */
        ...
        for (unsigned int i = 0; i < machinesQuantity; ++i) {
            ...
            while (true) {
                if ((machines[i]->GetCompletionTime()
                     + l_processes.front ()->GetProcessingTime())
                        < C) {
...
/*!
 * \brief Applies the simple LPT (assign longest unassigned job to the next free machine) algorithm to create a starting solution for the given instance stored in the Scheduler
 */
...
/*!
 * \brief Applies the STUPID algorithm to create a starting solution for the given instance stored in the Scheduler
 */
